...
module Sudoku.Solvers.SimplifiedLensStateWriter where
...
data Message = IsValueForCell Cell CellValue
    | IsNotValueForCell Cell CellValue 
...
handleIsNotValueForCellRcpt
    :: Cell -> CellValue -> MessageHandler ()
handleIsNotValueForCellRcpt self v = do
...
handleIsValueForContainerRcpt cell v self = do
    ...
    tell [ IsNotValueForCell cell v | cell <- Set.toList cells ]

handleIsNotValueForContainerRcpt
    :: Cell -> CellValue -> Container -> MessageHandler ()
handleIsNotValueForContainerRcpt cell v self = do
...
handlerForMessage (IsNotValueForCell cell v) = handleIsNotValueForCellRcpt cell v
...
data SimplifiedLSWSolver = SimplifiedLSWSolver deriving (Show)

instance Solver SimplifiedLSWSolver where
    type Msg SimplifiedLSWSolver = Message
    solve
        :: (MessageQueue q Message)
        => SimplifiedLSWSolver
        -> ([Message] -> q Message)
        -> Puzzle
        -> PuzzleResults
    solve SimplifiedLSWSolver mqw p =
